...
function SASActionButton_OnUpdate()
	if (not getglobal(this:GetName() .. "Icon"):IsVisible() or not SAS_Temp) then
		return ;
	end
...
function SASMinimap_OnEnter()
	if (SAS_SetsDropDown:IsVisible()) then
		return ;
	end
...
function SAS_BuildActionInfo(...)
	...
	while not arg[arg.n] and arg.n > 1 do
		arg.n = arg.n - 1;
	end
	if (arg.n < 2) then
		return ;
...
function tRank(rank)
	...
	for i in string.gfind(rank, "%d+") do
		return tonumber(i);
	end
end
...
function tLink(itemLink)
	...
	for num in string.gfind(itemLink, "(%d+):0:0:0") do
		return num;
	end
	return itemLink;
end
